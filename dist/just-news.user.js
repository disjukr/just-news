...
function checkUrl(pattern, url = window.location.href) {
    return new RegExp(escapeRegExp(pattern).replace(/\\\*/g, '.*')).test(url);
}
exports.checkUrl = checkUrl;
;
function here() {
    for (let site in sites_1.default) {
        for (let pattern of sites_1.default[site]) {
            if (checkUrl(pattern)) {
                return site;
            }
        }
...
;
main: {
    if (!reconstruct_1.reconstructable()) {
        break main;
    }
    waitWhilePageIsLoading().then(() => {
        const where = here();
        return require('./impl/' + where).default();
    }).then(article => reconstruct_1.reconstruct(article)).catch(e => {
...
function default_1() {
    return new Promise(resolve => {
        const created = (() => {
            ...
            return m ? new Date(((Date.now() / d | 0) - m[1]) * d - 1000 * 60 * 60 * 9 /* +09:00 */) : null;
        })();
        const result = {
