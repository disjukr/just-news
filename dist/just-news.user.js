...
// @include http://biz.khan.co.kr/khan_art_view.html*
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            if (impl.readyToParse) {
                yield impl.readyToParse();
            }
            else {
                yield util_1.waitDOMContentLoaded();
            }
            const article = impl.parse ? yield impl.parse() :
                impl.default ? yield impl.default() :
                    null;
            if (article == null)
                throw new Error('구현된 파싱 함수가 없습니다.');
            reconstruct_1.reconstruct(article, impl.cleanup || article.cleanup);
        }
        catch (e) {
...
function reconstruct(article, cleanup) {
    {
...
function parse() {
    return new Promise(resolve => {
        const created = (() => {
            ...
            if (date)
                return new Date(date[1] + '-' + date[2] + '-' + date[3] + 'T00:00:00+09:00');
            let d = 1000 * 60 * 60 * 24;
            const m = (() => {
                let m;
            ...
            return m ? new Date(((Date.now() / d | 0) - m[1]) * d -
                (1000 * 60 * 60 * 9 /* +09:00 */)) : null;
        })();
        const result = {
...
function default_1() {
    ...
    jews.content = util_1.clearStyles($('#cont_newstext')[0].cloneNode(true)).innerHTML;
    jews.reporters = [{
...
function default_1() {
    ...
    jews.subtitle = undefined;
    jews.content = util_1.clearStyles($('#DivPrint .view-con')[0].cloneNode(true)).innerHTML;
    jews.timestamp = {
...
const util_1 = require("../util");
exports.readyToParse = () => util_1.waitElement('#adtive');
exports.cleanup = () => $('#scrollDiv, iframe').remove();
function parse() {
    ...
    return {
        ...
        content: (() => {
            ...
            return util_1.clearStyles(articleBodyElement).innerHTML;
        })(),
        timestamp: (() => {
...
exports.cleanup = () => $('#scrollDiv').remove();
function parse() {
    ...
    return {
        title: $('.nwsti h3').text(),
        content: (() => {
            const article = $('#article')[0].cloneNode(true);
            $('.best_nw', article).remove();
            return util_1.clearStyles(article).innerHTML;
        })(),
...
function default_1() {
    ...
    jews.content = util_1.clearStyles(content).innerHTML;
...
function default_1() {
    ...
    jews.content = (function () {
        ...
        // $('.relatednews', content).remove();
        if (centerImage) {
            return util_1.clearStyles(centerImage.cloneNode(true)).innerHTML + util_1.clearStyles(content).innerHTML;
        }
        return util_1.clearStyles(content).innerHTML;
    })();
    jews.timestamp = (function () {
...
function default_1() {
    ...
    jews.content = util_1.clearStyles(document.querySelector('.news_text')).innerHTML;
...
function default_1() {
    ...
    jews.content = (function () {
        ...
        return util_1.clearStyles(content[0]).innerHTML;
    })();
    jews.timestamp = {
    ...
    jews.content = (function () {
        ...
        if (photo) {
            ...
            $('li > *', photo).toArray().forEach(function (item) {
                photoDiv.appendChild(util_1.clearStyles(item));
            });
            return util_1.clearStyles(photoDiv).innerHTML + util_1.clearStyles(content).innerHTML;
        }
        return util_1.clearStyles(content).innerHTML;
    })();
    jews.timestamp = {
...
function default_1() {
    return new Promise(resolve => {
        ...
        jews.content = (function () {
            ...
            if (articleImage) {
                header += util_1.clearStyles(articleImage.cloneNode(true)).innerHTML;
            }
            return header + util_1.clearStyles($('#articleText')[0].cloneNode(true)).innerHTML;
        })();
        jews.timestamp = {
            ...
            xhr.onreadystatechange = function () {
                if (this.readyState === (this.DONE || 4)) {
                    ...
                    imgJSON.forEach(function (v) {
                         ...
                         * </figure>
                         */
                        var fig = document.createElement('figure'), el = document.createElement('img'), caption = document.createTextNode(v.caption);
                        el.src = v.image;
                        fig.appendChild(el);
...
function default_1() {
    ...
    jews.content = (function () {
        ...
        $('script', d).remove();
        return util_1.clearStyles(d[0]).innerHTML;
    })();
    {
...
function default_1() {
    ...
    jews.timestamp = {
        created: new Date($('.infobox1 .num:first').text().replace(": ", "").replace(/\./g, '/')),
        lastModified: undefined
    };
    jews.reporters = (function () {
        var ret = [];
...
function default_1() {
    ...
    jews.content = (function () {
        ...
        if (figure) {
            ...
            figure = util_1.clearStyles(figure).innerHTML;
        }
        else {
            figure = '';
        }
        return figure + util_1.clearStyles(content).innerHTML;
    })();
...
function default_1() {
    ...
    var el_filter = function (el, obj, m) {
        ...
        if (m === true || m === undefined)
            f = function (v) { return v instanceof obj; };
        else
            f = function (v) { return !(v instanceof obj); };
        return _filter(el, f);
    };
...
function default_1() {
    return new Promise(resolve => {
        let jews = {};
        var category = $('.article-title .category').text();
        if (category === '뉴스 타임라인') {
        ...
        ai.find('.meta-text').each(function (v) {
            ...
            if (a !== null) {
                if (a[1] === '발행')
                    jews.timestamp.created = new Date(a[2] + 'T' + a[3] + '+09:00'); // ISO 8601
                else if (a[1] === '수정')
                    jews.timestamp.lastModified = new Date(a[2] + 'T' + a[3] + '+09:00'); // ISO 8601
            }
        });
        jews.reporters = function () {
            var reporters = undefined, email = undefined, aiw = ai.find('.writer'), abm = $('.article-bottom-meta');
            if (abm.find('.writer').length > 0)
                reporters = abm.find('.writer')[0].textContent.trim();
            else if (aiw.length > 0)
                reporters = aiw[0].textContent.trim();
...
function default_1() {
    ...
    jews.content = (function () {
        ...
        return util_1.clearStyles(newsContent).innerHTML;
    })();
    jews.timestamp = {
    ...
    jews.reporters = (function () {
        ...
        return [{
                name: util_1.clearStyles(reporters).innerHTML,
                mail: undefined
            }];
    })();
    return jews;
...
function default_1() {
    ...
    $('.v_days').text()
        ...
        if (p1 === '입력')
            jews.timestamp.created = new Date(ts);
        else if (p1 === '수정')
            jews.timestamp.lastModified = new Date(ts);
    });
    jews.reporters = [];
    return jews;
}
...
function default_1() {
    ...
    jews.content = util_1.clearStyles(document.getElementById('article_txt')).innerHTML;
    jews.timestamp = { created: undefined, lastModified: undefined };
...
function default_1() {
    ...
    jews.subtitle = undefined;
    jews.timestamp = {
        created: new Date(document.querySelector('.sub_contents>.article_cont .article_tit>p').textContent.replace('입력', '').trim().replace(/\s+/, 'T') + '+09:00'),
        lastModified: undefined
    };
    jews.content = util_1.clearStyles(document.querySelector('#ct>div.article_word')).innerHTML;
    jews.reporters = [];
    jews.cleanup = function () {
...
function default_1() {
    ...
    function parseDate(t) {
        var date = /(\d{4})\-(\d{2})\-(\d{2})/.exec(t), m = /(\d)일전/.exec(t), d = 1000 * 60 * 60 * 24;
        if (date !== null)
            return new Date(t.replace(/-/g, '/').replace(/[가-힣]/g, '').replace(/^\s+/g, ''));
        else {
            if (m === null) {
                if ((m = /(\d+)시간전/.exec(t)) !== null)
                    d /= 24;
                else if ((m = /(\d+)분전/.exec(t)) !== null)
                    d /= 24 * 60;
            }
            if (m !== null)
                return new Date(((Date.now() / d | 0) - (m[1] | 0)) * d - 1000 * 60 * 60 * 9 /* +09:00 */);
        }
    }
    jews.timestamp = {
...
function default_1() {
    ...
    jews.timestamp = ({
        created: new Date($('.articleHeadlineBox .dateStamp')[0].textContent.replace(/\s*KST\s*$/, ' +0900').replace(/(\d+)\.?\s+([a-z]{3})[a-z]+\s+(\d+)\s*,\s*/i, '$1 $2 $3 ')),
        lastModified: undefined
    });
    jews.reporters = (function () {
...
function default_1() {
    ...
    jews.content = (function () {
        var content = $('#viewcontent_inner, #viewcontent')[0].cloneNode(true);
        ...
        return (image ? util_1.clearStyles(image).innerHTML : '') + util_1.clearStyles(content).innerHTML;
    })();
    jews.timestamp = (function () {
...
function default_1() {
    ...
    jews.reporters = (function () {
        ...
        reporters.each(function (el) {
            ...
            if (mail_el.attr('href') !== null)
                mail = mail_el.attr('href').replace(/mailto:/, '');
            list.push({
                name: name,
                mail: mail
            });
...
function default_1() {
    ...
    jews.subtitle = undefined;
    jews.content = util_1.clearStyles($('#article-body')[0].cloneNode(true)).innerHTML;
    jews.timestamp = {
        created: undefined,
        lastModified: undefined
    };
...
function default_1() {
    ...
    jews.content = util_1.clearStyles($('#articleText')[0].cloneNode(true)).innerHTML;
    jews.timestamp = (function () {
...
function default_1() {
    ...
    jews.content = util_1.clearStyles($('#news_body_area')[0].cloneNode(true)).innerHTML;
    jews.timestamp = {
...
function default_1() {
    ...
    {
        let articleBody = util_1.clearStyles($('.article-text')[0].cloneNode(true));
...
function default_1() {
    ...
    jews.content = util_1.clearStyles(content[0]).innerHTML;
    jews.timestamp = (function () {
...
function default_1() {
    ...
    jews.content = (function () {
        ...
        return mainImageContent + mainVideoContent + util_1.clearStyles(content[0]).innerHTML;
    })();
    jews.timestamp = {
